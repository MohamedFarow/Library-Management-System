...
GO
	...
	with row_num_cte as (
	select *, ROW_NUMBER() over( partition by book_loans_bookid, book_loans_branchid order by book_loans_loansid) as row_num
	 from tbl_book_loans  --order by book_loans_cardno
	 ) --order by row_num 
	 select * from row_num_cte where row_num >1 order by book_loans_cardno
...
-- if OBJECT_ID(N' dbo.BooksLoanedOut',N'U') is null CREATE PROC dbo.BooksLoanedOut
--(@BooksCheckedOut INT = 5) AS
...
---How many copies of the book "The Lost Tribe" are there in the Sharpstown branch?
SELECT SUM(book_copies_No_Of_Copies)
...
AND tbl_book.book_Title = 'The Lost Tribe';

----What is the total number of books loaned out from each library branch?
select library_branch_branchname, COUNT(*) as TotalLoans
from tbl_book_loans
...
---What is the name and address of each borrower with more than 5 books checked out?
select borrower_BorrowerName,borrower_BorrowerAddress, COUNT(borrower_borrowername)
from tbl_borrower 
...
having count(*) > 5
---What is the most popular book genre based on loans?
select top 5 book_Title, count(*) as TotalLoans
from tbl_book 
join tbl_book_loans on tbl_book.book_BookID = tbl_book_loans.book_loans_BookID 
group by tbl_book.book_Title
...
where tbl_book_loans.book_loans_DueDate < GETDATE()

--What is the average number of days that books are checked out?
select AVG(DATEDIFF(day, book_loans_DateOut,book_loans_DueDate))
from tbl_book_loans
---Which books have never been checked out?
select book_title from tbl_book
left join tbl_book_loans on tbl_book.book_BookID = tbl_book_loans.book_loans_BookID
where tbl_book_loans.book_loans_BookID is null

--- Which author has the most books in the library?

select top 1 book_authors_AuthorName, COUNT(*) as BookCount
from tbl_book_authors
join tbl_book on tbl_book_authors.book_authors_AuthorID = tbl_book.book_BookID
...
select book_title, book_copies_no_of_copies,library_branch_branchname
	 from tbl_book_authors
	 inner join tbl_book on tbl_book_authors.book_authors_AuthorID = tbl_book.book_BookID
	 inner join tbl_book_copies on tbl_book_authors.book_authors_AuthorID = tbl_book_copies.book_copies_BookID
	 inner join tbl_library_branch on tbl_book_copies.book_copies_BranchID = tbl_library_branch.library_branch_BranchID
	  where library_branch_BranchName = 'Central' AND book_authors_AuthorName = 'Stephen King'

-- How many copies of the book titled "The Lost Tribe" are owned by the library branch whose name is "Sharpstown"? */
select book_copies_branchid,book_title,library_branch_BranchName
from tbl_book_copies
...
--How many books does each publisher have in the library?
select publisher_PublisherName, COUNT(*)

from tbl_publisher 
join tbl_book on tbl_publisher.publisher_PublisherName = tbl_book.book_PublisherName
group by publisher_publishername order by publisher_PublisherName desc

--What is the most common borrower address?
select * from tbl_borrower
select   borrower_BorrowerAddress, count(*) as BorrowerCount
from tbl_borrower
group by borrower_BorrowerAddress
order by BorrowerCount desc

--How many copies of each book title does the library have?
select book_title,
--library_branch_BranchName,
sum(book_copies_No_Of_Copies) as No_of_Copies from tbl_book 
join tbl_book_copies on tbl_book.book_BookID = tbl_book_copies.book_copies_BookID
--join tbl_library_branch on tbl_book_copies.book_copies_BranchID = tbl_library_branch.library_branch_BranchID
group by book_Title --library_branch_BranchName

--Which books are due to be returned today?
select * from tbl_book_loans
update tbl_book_loans set book_loans_DueDate = cast(getdate() as date) where book_loans_LoansID = 20
select book_title,count(book_title) as Num from tbl_book
join tbl_book_loans on tbl_book.book_BookID = tbl_book_loans.book_loans_BookID
where tbl_book_loans.book_loans_DueDate = cast(GETDATE() as date)
group by book_Title
order by num desc

--What is the most popular book at each branch?
select book_title,library_branch_branchname,COUNT(*) as CheckOut from tbl_library_branch
...
--How many borrowers have checked out more than 10 books in the last year?

select tbl_borrower.borrower_BorrowerName, count(borrower_BorrowerName) as CheckedOut
from tbl_borrower
inner join tbl_book_loans on tbl_borrower.borrower_CardNo = tbl_book_loans.book_loans_CardNo
where tbl_book_loans.book_loans_DateOut > dateadd(year, -8, getdate())
group by borrower_BorrowerName
having count(tbl_book_loans.book_loans_BookID) > 10
--What is the average number of days overdue per book?
select AVG(datediff(day,book_loans_duedate, getdate())) AS AVGNumber
from tbl_book_loans
where GETDATE() >  book_loans_DueDate

--Which author has the most distinct readers? 
select book_authors_AuthorName, count(distinct tbl_book_loans.book_loans_LoansID) As DistinctReaders
from tbl_book_authors
join tbl_book on tbl_book_authors.book_authors_AuthorID =  tbl_book.book_BookID
join tbl_book_loans on tbl_book.book_BookID = tbl_book_loans.book_loans_BookID
group by book_authors_AuthorName
order by DistinctReaders desc
